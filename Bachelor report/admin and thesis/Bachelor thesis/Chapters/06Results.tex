...
Individual anomalies are shown as a scatter plot (Section~\ref{sec:scatter_plot}). The visualization shows the points colored based on signal strength, from red to green.
\begin{figure}[H]
    \centering
...
The filter system allows the rendered dataset to be narrowed by anomaly type, \acrshort{mmsi}, and a start and end date.
% TODO (you): add a screenshot of the filter panel (Figure ref), and ideally a before/after pair
% showing the rendered dataset shrinking when a filter is applied.

\subsection{Temporal Filtering}
A date-time range selector allows the visualization to be restricted to a chosen time period. The selector reuses an existing expandable React calendar component, which diverged from and simplified the date picker shown in the wireframe.
% TODO (you): add a screenshot of the date picker (Figure ref) and describe what the user sees when a range is applied.

\subsection{Satellite Visualization}
Satellite connections are displayed as a layer that appears only when a data point is clicked. The satellite-orbit \acrshort{api} is access-restricted and requires credentials, so relying on it would prevent others from testing this public prototype. Each satellite point is therefore approximated by finding the closest position in time within a 24-hour window of the anomaly.
...
% TODO (you): add a screenshot of the satellite layer appearing on click (Figure ref).
...
Performance was observed during normal use of the application, primarily at the default zoom level with two to three page refreshes per observation, on the two laptops used by the group for ordinary schoolwork. Each visualization type was observed at different zoom levels and across different areas of the map. The application was tested with a main dataset of 10,000 points, and additionally with larger datasets of up to 100,000 and one million points generated through backend configuration files. Across these tests the application rendered the requested data within roughly a second of the backend request, with most of that time spent sending the request and receiving the response, keeping the interface within a responsive range.
...
% TODO (you): run a quick timing test, then replace "profound effect"/"very inefficient" with the
% actual point count where the un-clustered scatter became laggy versus the clustered view staying smooth.
