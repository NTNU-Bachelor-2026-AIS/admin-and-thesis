...
Satellite connections are displayed as a layer that appears only when a data point is clicked. Because full access to an external \acrshort{api} for computing a satellite's precise daily position was not available, each satellite point is approximated by finding the closest position in time within a 24-hour window of the anomaly.
