...
% SUGGESTION (Method/Results bleed — your call): the 40-pixel aggregation mechanism (how clusters are
% computed) is Method 3.5 content. Move the preserved sentence below to Method; Results should show
% what the figure shows. The clustering performance effect is reported in Section~\ref{sec:effect_of_clustering_results}.
% MECHANISM (preserved for moving to Method): "Clusters are calculated using a radius that counts the
% points inside a circle of a defined radius, 40 pixels in this project. Points inside the radius are
% aggregated and shown as a single point. Recalculation happens on zoom and drag, where clusters split
% into smaller parts as points move outside the radius."
...
% SUGGESTION (Method/Results bleed — your call): the "sum of points inside a 40-pixel radius" sentence
% above is mechanism (Method 3.5); consider moving it to Method.
...
The filter system allows the rendered dataset to be narrowed by anomaly type, MMSI, and a start and end date.
...
Satellite connections are displayed as a layer that appears only when a data point is clicked. Because full access to an external API for computing a satellite's precise daily position was not available, each satellite point is approximated by finding the closest position in time within a 24-hour window of the anomaly.
