...
\gls{maplibre} initializes itself upon application boot, in a bright style used for readability for users. After its initialization, it is responsible for interaction, in dragging and zooming the map. Interaction leads to states being updated that other parts of the program can use, such as clusters being updated through zooming, and map location. Visualization functionality is something \gls{maplibre} has, being able to natively visualize scatterplots and other layers. 
...
\begin{figure}[H]
    \centering
    \includegraphics[height=0.32\textheight,keepaspectratio]{Figures/full_application_image.png}
...
The anomalies and anomaly groups are generated by the \gls{docker} backend, with the amount of anomalies set through a configuration file. Specific commands to generate anomalies and build the database are used, leading to a database filled with an amount of points set by the developer. The \gls{docker} backend running allows for the frontend to send requests to \acrshort{api} endpoints to request data, while the frontend itself runs through localhost.
...
\end{figure}
Anomaly data is completely integrated in a FeatureCollection, with the individual features being anomaly groups. These features are individually processed in the frontend, as a dataset, through \gls{deckgl}.
...
A date-time range selector allows the visualization to be restricted to a chosen time period. The selector reuses an existing expandable \gls{react} calendar component, which diverged from and simplified the date picker shown in the wireframe, as shown in Figure~\ref{fig:date_picker_image}.
...
\bottomrule
\end{tabular}
\caption[Backend API response times]{Backend \acrshort{api} response times for the anomaly-groups endpoint across two dataset sizes, measured from \gls{docker} logs. The unfiltered range reflects initial cold requests versus subsequent requests benefiting from database caching. Filtered results used a single anomaly type filter.}
