...
The finished product meets the main requirement the customer requested, with the most-requested features delivered and three planned features not developed in agreement with the customer. The quality of the finished project as a concept test for new visualization methods was noted in the customer feedback presented in Section~\ref{sec:customer_feedback}.
...
The system architecture, with \gls{deckgl} overlaid on \gls{maplibre} as the base map and a \gls{docker}-based backend providing data, worked well throughout development. The modular approach led to \gls{maplibre} being separated from the \gls{deckgl} implementation (Section~\ref{sec:deckgl}), with the map itself responsible only for  map-related functionality such as zooming and movement. This separation simplified the overall complexity and produced cleaner code with a clear separation of concerns. Achieving this required a period of refactoring tightly coupled code, particularly where the map itself contained functionality unrelated to its task. 

The architecture choice was partially guided by the customer, whose request to use \gls{react} and other technologies already in use within their infrastructure shaped the group's decisions, alongside the customer providing the backend. \gls{maplibre} was suggested in the project document and was ultimately selected as the map provider, supported by its open-source nature and use of vector tiles (Section~\ref{sec:vector_tiles}).
...
The choice of map provider had both advantages and disadvantages. \gls{maplibre} is a fork of the now closed-source Mapbox, which meant that documentation was less comprehensive than required, and that more detailed information for equivalent code often had to be retrieved from the Mapbox website. As a result working with \gls{maplibre} required more time spent understanding the library than was originally anticipated, delaying implementation beyond what had been planned.

\gls{deckgl} (Section~\ref{sec:deckgl}) proved to be an effective choice for visualization, providing all functionality needed to implement the customer's requested features. Layering through \gls{deckgl} allowed straightforward integration with \gls{maplibre} by utilizing a MapBox-style overlay. Since \gls{maplibre} is a fork of MapBox, no additional integration code was required beyond what would have been needed for MapBox itself. 
...
The use of \gls{react} with \gls{typescript} enforced stronger type safety than plain \gls{react}, but introduced an initial learning curve. The group's lack of experience with the coding standards needed for \gls{react} and \gls{typescript}, led to development time being spent on learning the basics as well as more advanced tools such as Context, which was ultimately used to simplify the code.  
...
The code followed the software engineering principles described in Section~\ref{sec:software_principles_theory}, including separation of concerns, cohesion, and the single responsibility principle. One example that happened in the project was when the group refactored the map container to remove \gls{deckgl} visualization logic, which meant that the component became responsible only for map-related functionality such as zoom and pan, which directly applied the separation of concerns principle (Section~\ref{sec:SOC_theory}). \gls{react}'s component system supported this by allowing each visualization layer to be defined as a separate functions with singular purposes.

The lack of prior experience with \gls{react}-specific patterns, such as Context for shared state, did lead to some initial design decisions that required refactoring later in the project. However, identifying and correcting these through repeated refactoring improved code quality progressively throughout development, and the final codebase is more maintainable as a result.
...
Clustering, implemented as described in Section~\ref{sec:clustering_method} and shown in Section~\ref{sec:effect_of_clustering_results}, produced a measurable performance improvement compared to rendering all points as a scatter plot. At 100,000 anomalies in 15,331 groups, the clustered view renders only the cluster markers visible within the current viewport rather than all 100,000 points simultaneously, significantly reducing the rendering load while also improving readability. 
...
The backend was provided by the customer with most of the endpoints required by the project already implemented, supported by comprehensive Swagger documentation. \gls{docker} simplified the process of running the backend, requiring only a few commands to start and fill the database with test data. 
...
Data handling through \acrshort{geojson}, proved a good choice for both the backend and frontend, with the libraries used already supporting the data. The data is parsed through a \gls{deckgl} layer that reads properties and geometry from each GeoJSON feature, which proved to be an effective design allowing straightforward access to all anomaly data fields. A tradeoff of using \acrshort{geojson} is that the frontend must have prior knowledge of the data structure to correctly access those properties, creating a dependency on a stable schema. A further issue encountered during development was asynchronous timing errors, where components attempted to access data before the API response had resolved, which required adding appropriate handling to ensure data was available before rendering. 
...
In relation to sprint documentation, the sprint artifacts provided traceable project documentation, which served as a reference when writing the report while ensuring important information was not lost in miscommunication.
...
Slack provided an efficient channel for direct communication with the customer, enabling rapid resolution of short technical questions. The product owner's written evaluation noted over 250 messages exchanged through Slack by 11 May, and acknowledged that the group took initiative in arranging progress meetings to present ongoing development. The customer's domain expertise allowed complex questions to be resolved within short discussions rather than through a long discussion. Communication with the supervisor was handled primarily through email, with in-person consultations used for urgent issues, as most work was conducted on campus.
...
Testing was conducted through manual observation instead of automated tests or controlled benchmarks. Performance results were therefore mostly observed rather than precisely measured, which made the absence of an automated test suite limit confidence in catching problems as the codebase develops. For a prototype at this scope, manual testing was a practical choice that allowed flexible exploration of edge cases, but a future production version would benefit from automated integration tests covering the \acrshort{api} endpoints and frontend layer logic.
