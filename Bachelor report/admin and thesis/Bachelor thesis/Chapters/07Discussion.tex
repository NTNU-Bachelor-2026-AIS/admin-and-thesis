...
This subsection evaluates the responsiveness observed during manual testing.
% SUGGESTION (write in your own words using the data below — this section is currently empty prose):
% The backend showed acceptable response times at the primary test size of 100,000 anomalies
% (196--461~ms unfiltered, 55--62~ms filtered), but scaled to 2.0--3.1~s unfiltered at one million
% anomalies (Table~\ref{tab:api_response_times}). For the intended use case of a concept prototype
% on the customer's existing data volumes, the 100k figures are more representative.
% On the frontend, the main responsiveness cost was switching from the clustered view to the anomaly
% group view, where signal-strength lines are computed — add a rough wait time here (under a second?
% 1--2 seconds?). Heat maps were the slowest layer, recalculating area weights on every zoom and move;
% this is bounded by Deck.gl's own implementation, so no further gain was available without a
% fundamentally different approach such as a clustered heat map.
% Filtering and layer switching were otherwise responsive, changing as soon as input was submitted.
...
% SUGGESTION (self-reflection / lessons learned — high value for criterion 5; write in your own words):
% consider adding a short reflective paragraph acknowledging that, in retrospect, the team should have
% engaged more carefully with the project requirements and the grading criteria earlier, and used them
% to focus the work. Frame it as a lesson learned rather than self-criticism: clearer early alignment on
% requirements and assessment criteria would have improved prioritization and reduced time spent on work
% that was later descoped. This pairs naturally with the team-capacity change and the reactive scope
% management discussed below, and honest reflection of this kind tends to raise the assessment score.
...
% SUGGESTION (reconcile with the real sprint structure in Results 4.1; write in your own words):
%  - The cadence was weekly EXCEPT the final two sprints, which each ran two weeks (15-16 and 17-18) as
%    the team moved toward the report period. Adjust "the group used weekly sprints" to reflect this.
%  - Small lessons-learned tie-in: the first two sprints were documented informally before the team
%    found and adopted the official sprint templates, after which documentation was consistent. This
%    supports the earlier reflection that engaging with the expected process/criteria sooner would have
%    helped.
...
% SUGGESTION (reflect on the 3->2 team change — high value for criterion 5; write in your own words):
% the project started with three members and continued with two after one dropped out. Reflect on the
% impact: reduced capacity raised the per-member workload and led to a deliberate scope reduction toward
% the most useful prototype features (data visualization) and away from report generation and hexagonal
% binning; the two-person daily stand-ups and shared responsibilities became more important for keeping
% momentum. Tie this to the scope decisions made with the customer (see Scope Management below).

\subsection{Communication and Meeting Structure}

Slack provided an efficient channel for direct communication with the customer, enabling rapid resolution of short technical questions.
% SUGGESTION (add the customer's quantified observation as evidence — your wording): the product owner's
% written feedback records 250+ Slack messages exchanged by 11 May, and notes that the group took the
% initiative to arrange progress meetings to show development. Citing this customer-sourced detail backs
% up the "communication was effective" claim with a concrete number instead of an assertion.
The customer's domain expertise allowed complex questions to be resolved within short discussions rather than through a long discussion. While communication with the supervisor was handled primarily through email, with in person consultations used for urgent issues, because most work was conducted on campus.
...
% SUGGESTION (link scope reduction to the team change — write in your own words): note that the move
% from three to two members directly drove the scope reduction. In agreement with the customer, the team
% prioritized the data-visualization features that gave the most prototype value and dropped report
% generation and hexagonal binning. Framing the cut features as a deliberate, capacity-based decision
% (made with the customer) reads as sound engineering judgement rather than a shortfall.
