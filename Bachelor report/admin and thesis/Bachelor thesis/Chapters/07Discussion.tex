...
Client-side filtering on top of an already-filtered dataset would have been possible, however that solution would have required keeping the original dataset in memory and rerunning the filter logic on the frontend to further filter the data the group allready has. However that would be inefficient at the dataset sizes used in this project, because the containerized backend responds in the millisecond range and routing filter operations to the backend to then be handled by the query proved faster than client-side re-filtering. Because of this the rendered dataset always reflects the currently active filters directly, without the need to keep a seperate "full dataset" on the frontend. When users need to drill down further within the active dataset, the sidebar search component provides this functionality locally without requiring an additional backend request.
...
The performance of the system is highly dependent on hardware specifications, with the GPU having a particularly strong. WebGL benefits significantly from more capable GPUs \ref{sec:hardware_acceleration}), meaning that performance is bottlenecked on weaker systems. 
