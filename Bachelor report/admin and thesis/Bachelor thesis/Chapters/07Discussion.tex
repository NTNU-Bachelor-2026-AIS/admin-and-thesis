...
% DRAFT (move the evaluative judgments here from Results 4.8 "System Responsiveness" and
% "Effect of Clustering on Performance", then add approximate numbers):
% Overall the system stayed responsive during use. The main cost was switching from a clustered view to
% an anomaly-group view, where the signal-strength lines are computed, causing a brief slowdown before
% settling. Heat maps were the slowest layer, recalculating area weights on every zoom and move; this is
% bounded by Deck.gl's implementation, so little further gain was available without a different approach
% (e.g. a clustered heat map). Clustering versus a plain scatter plot: <state the point count where the
% plain scatter became laggy versus the clustered view staying smooth, once measured>. 
