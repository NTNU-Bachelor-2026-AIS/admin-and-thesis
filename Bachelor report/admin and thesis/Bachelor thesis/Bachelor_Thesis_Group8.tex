...
\begin{document}
% Two blank pages — reserved for the NTNU submission system to insert the official cover pages.
% Each needs both \null (so the otherwise-empty page is actually shipped) and \thispagestyle{empty}
% (so the book class's default "headings" style does not print a page number in the top corner).
...
\markboth{}{} % clear LIST OF TABLES mark

\pagestyle{plain}

% Use \let\cleardoublepage\clearpage inside a group so fncychap cannot insert a blank page
% before Abbreviations or Terms (which would show a spurious "LIST OF TABLES" header).
...
\chapter{Discussion}
\input{Chapters/07Discussion}
\cleardoublepage


\chapter{Conclusions}
\input{Chapters/08Conclusion}
\cleardoublepage

\chapter{Societal Impact}
\input{Chapters/09SocietalImpact}
\cleardoublepage
